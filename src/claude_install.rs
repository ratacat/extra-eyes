...
const INSTALLED_EVENTS: [&str; 2] = ["UserPromptSubmit", "Stop"];
...
fn installed_command(eyes_bin: &Path, event: &str) -> String {
    format!(
        "{} hook claude-code --integration extra-eyes --event {event} --project .",
    ...
    "UserPromptSubmit": [
      {
        ...
        assert_eq!(first.installed_events, ["UserPromptSubmit", "Stop"]);
        let second = install_claude_hooks(&settings, &eyes_bin).unwrap();
        assert_eq!(second.installed_events, ["UserPromptSubmit", "Stop"]);
